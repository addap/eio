...
let run_bench ~domain_mgr ~clock ~n_send_domains ~n_iters ~capacity =
    ...
    Metric.create ("time_per_iter/" ^ (string_of_int n_send_domains) ^ "_" ^ (string_of_int n_iters) ^ "_" ^ (string_of_int capacity))
      (`Numeric (1e9 *. time_per_iter)) "ns/iter" "time_per_iter/n_send_domains/n_iters/capacity"
