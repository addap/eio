...
let await t =
  ...
  | Resolved x ->
    ...
    x
  | Unresolved b ->
    (* a.d. if the promise is unresolved, arrange for enqueue to be called when the broadcast is triggered. *)
    Suspend.enter (fun ctx enqueue ->
        match Broadcast.suspend b (fun () -> enqueue (Ok ())) with
        (* a.d. racy stuff (all this re-checking probably has to be done due to being lock-free) *)
        | None -> ()  (* We got resumed immediately *)
        (* a.d. the request can be used to remove yourself from the broadcast. *)
        | Some request ->
          match Atomic.get t.state with
          | Resolved _ ->
          ...
          | Unresolved _ ->
            ...
            (* set the cancel function to remove yourself from the broadcast if the fiber gets cancelled while waiting. *)
            Cancel.set_cancel_fn ctx (fun ex ->
                if Broadcast.cancel request then enqueue (Error ex)
                (* else already resumed *)
                (* a.d. the fiber is already resumed but will fail quickly because the cancellation context is set to cancelled. *)
              )
      );
    (* a.d. we only resume from the suspend if the promise was resolved. *)
    match Atomic.get t.state with
    | Resolved x ->
      Ctf.note_read t.id;
      x
