...
type _ Effect.t += Fork : Cancel.t * (unit -> unit) -> unit Effect.t
...
let fork_promise f =
  ...
  let new_fiber = Cancel.make ~protected:false in
  let p, r = Promise.create_with_id (Cancel.tid new_fiber) in
  fork_raw new_fiber (fun () ->
      match f () with
      | x -> 
        Ctf.note_resolved (Cancel.tid new_fiber) ~ex:None;
        Promise.resolve_ok r x
      | exception ex -> 
        Ctf.note_resolved (Cancel.tid new_fiber) ~ex:(Some ex);
...
(* a.d. 
   ...
   2. perform the Form effect to register the fiber in the scheduler.
   3. cleanup the whole switch if the fiber causes an exception. *)
let fork f =
  let (cc, _) = fork_promise f in
  cc

(*
(* a.d. creates a new switch and forks all the fibers using it. *)
let all xs =
  Switch.run @@ fun sw ->
...
(* a.d. TODO you wait for yourself being canceled? When is that practical? *)
let await_cancel () =
  Suspend.enter @@ fun fiber enqueue ->
  (* a.d. TODO how is this cancel_fn used? *)
