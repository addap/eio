module Fiber_context = Eio.Thin.Cancel
module Lf_queue = Eio_utils.Lf_queue

exception Deadlock_detected 

let traceln = Eio.ThinStd.traceln
...
let run main =
  ...
  let rec fork ~new_fiber:fiber fn =
    ...
    Effect.Deep.match_with fn ()
      { retc = (fun () -> 
        traceln "exited fiber successfully";
        ...
        effc = fun (type a) (e : a Effect.t) : ((a, exit) Effect.Deep.continuation -> exit) option ->
          match e with
          | Eio.Thin.Private.Effects.Suspend f -> Some (fun k ->
              (* traceln "Handling Suspend effect";           *)
          ...
          | Eio.Thin.Private.Effects.Fork (new_fiber, f) -> Some (fun k ->
              (* traceln "Handling Fork effect";           *)
          ...
          | Eio.Thin.Private.Effects.Get_context -> Some (fun k ->
              (* traceln "Handling GetContext effect";           *)
